...
namespace MDA.EventStore.SqlServer
{
    public class SqlServerDomainEventStore : IDomainEventStore
    {
        ...
        public async Task<AsyncResult<DomainEventAppendResult>> AppendAllAsync(DomainEventStream eventStream)
        {
            ...
            {
                _logger.LogError("Append all domain events associated with a aggregation has sql exception.", ex);
            ...
            {
                _logger.LogError("Append all domain events associated with a aggregation has unknown exception.", ex);

                return new AsyncResult<DomainEventAppendResult>(AsyncStatus.Failed);
            }
        }

        public async Task<AsyncResult<DomainEventAppendResult>> AppendAsync(DomainEvent domainEvent)
        {
        ...
        public async Task<AsyncResult<int>> CountStoredEventsAsync()
        {
            ...
            {
                _logger.LogError($"the {nameof(CountStoredEventsAsync)} has a sql exception", ex);

                return new AsyncResult<int>(AsyncStatus.Failed);
            }
            catch (Exception ex)
            {
                _logger.LogError($"the {nameof(CountStoredEventsAsync)} has a unknown exception", ex);

                return new AsyncResult<int>(AsyncStatus.Failed);
            }
        }

        public async Task<AsyncResult<IEnumerable<DomainEvent>>> GetAllAggregateStoredEventsBetweenAsync(string aggregateRootId, string aggregateRootTypeName, int lowSequence, int highSequence)
        {
            ...
            try
            {
                using (var connection = new SqlConnection(_options.ConnectionString))
                {
                    ...
                    return new AsyncResult<IEnumerable<DomainEvent>>(AsyncStatus.Success, events);
                }
            }
            catch (SqlException ex)
            {
                _logger.LogError($"the {nameof(GetAllAggregateStoredEventsBetweenAsync)} has a sql exception, aggregateRootId: {aggregateRootId},aggregateRootTypeName: {aggregateRootTypeName},LowSequence: {lowSequence},HighSequence: {highSequence}", ex);

                return new AsyncResult<IEnumerable<DomainEvent>>(AsyncStatus.Failed);
            }
            catch (Exception ex)
            {
                _logger.LogError($"the {nameof(GetAllAggregateStoredEventsBetweenAsync)} has a unknown exception, aggregateRootId: {aggregateRootId},aggregateRootTypeName: {aggregateRootTypeName},LowSequence: {lowSequence},HighSequence: {highSequence}", ex);

                return new AsyncResult<IEnumerable<DomainEvent>>(AsyncStatus.Failed);
            }
        }

        public async Task<AsyncResult<IEnumerable<DomainEvent>>> GetAllAggregateStoredEventsSinceAsync(string aggregateRootId, string aggregateRootTypeName, int sequence)
        {
            Assert.NotNullOrEmpty(aggregateRootId, nameof(aggregateRootId));
            Assert.NotNullOrEmpty(aggregateRootTypeName, nameof(aggregateRootTypeName));
            ...
            {
                _logger.LogError($"the {nameof(GetAllAggregateStoredEventsSinceAsync)} has a sql exception, aggregateRootId: {aggregateRootId},aggregateRootTypeName: {aggregateRootTypeName},EventSequence: {sequence}", ex);

                return new AsyncResult<IEnumerable<DomainEvent>>(AsyncStatus.Failed);
            }
            catch (Exception ex)
            {
                _logger.LogError($"the {nameof(GetAllAggregateStoredEventsSinceAsync)} has a unknown exception, aggregateRootId: {aggregateRootId},aggregateRootTypeName: {aggregateRootTypeName},EventSequence: {sequence}", ex);
