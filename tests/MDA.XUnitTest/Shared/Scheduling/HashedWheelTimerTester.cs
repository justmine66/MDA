...
namespace MDA.XUnitTest.Shared.Scheduling
{
    public class HashedWheelTimerTester
    {
        ...
        {
            var timer = new HashedWheelTimer(TimeSpan.FromMilliseconds(100));
            var barrier = new CountdownEvent(1);

            var timeout = timer.NewTimeout(
                new ActionTimerTask(
                    t =>
                    {
        ...
        public void TestStopTimer()
        {
            ...
            latch.Wait();
            // "Number of unprocessed timeouts should be 0"
            Assert.Equal(0, timerProcessed.StopAsync().Result.Count);
        ...
        public void TestExecutionOnTime()
        {
            ...
            var timer = new HashedWheelTimer(TimeSpan.FromMilliseconds(tickDuration));
            var queue = new BlockingCollection<PreciseTimeSpan>();

            var scheduledTasks = 10000;
            for (var i = 0; i < scheduledTasks; i++)
            {
                var start = PreciseTimeSpan.FromStart;
                timer.NewTimeout(
                    new ActionTimerTask(
                        t =>
                        {
                            queue.Add(PreciseTimeSpan.FromStart - start);
                        }),
            ...
            for (var i = 0; i < scheduledTasks; i++)
            {
                var delay = queue.Take().ToTimeSpan().TotalMilliseconds;
